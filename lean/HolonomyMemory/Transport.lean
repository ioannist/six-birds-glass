...
def currentTransport
  ...
  Quotient.lift
    ...
    (fun _ _ hEq => Quotient.sound (currentEventEquiv_push T γ hEq))
...
@[simp] theorem currentTransport_mk
    ...
    (γ : T.Continuation i j)
    (h : T.History i) :
    currentTransport T γ (Quotient.mk (CurrentSetoid T i) h) =
...
theorem predictiveToCurrent_commutes_with_transport
    ...
    currentTransport T γ (predictiveToCurrent T q) =
