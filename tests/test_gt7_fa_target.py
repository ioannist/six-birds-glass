...
def test_fa_target_exact_rows_match_registry_cells() -> None:
    artifact = _load_artifact()
...
def test_fa_target_mc_spotcheck_and_cross_validation() -> None:
    artifact = _load_artifact()
    rows = artifact["payload"]["hump_surface_mc_spotcheck"]["rows"]
    cross_validation = artifact["payload"]["n10_cross_validation"]

    assert len(rows) == 7
    assert {row["N"] for row in rows} == {16}
    assert len(cross_validation) == 2
    assert all(entry["pass"] is True for entry in cross_validation)


def test_fa_target_cell_ids_match_predictions_registry() -> None:
    artifact = _load_artifact()
    ...
    for row in sample_rows:
        for readout_id, cell_id in row["cell_ids"].items():
            ...
            assert cell_id == _expected_cell_id("fa", readout_id, row)


def _load_artifact() -> dict:
    return _load_json("artifacts/gt7_constants/fa_target.json")
